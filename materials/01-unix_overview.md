...
While clicking buttons and menus on a GUI feels natural at first, imagine doing the same task hundreds of times – your hand would get tired and you'd likely make mistakes!
Imagine the following task: for a literature search, you have to extract the authors' list contained in the third line of thousands of text files and count how many publications each author has.  
...
The Unix shell is both a **command-line interface** (CLI) and a **scripting language**, allowing such repetitive tasks to be done automatically and quickly.
...
While a GUI presents you with choices to select, CLI choices are not automatically presented to you, so you must learn a few commands like new vocabulary in a new language.
...
Familiarity with the shell is near-essential to run a variety of specialised tools and resources, including high-performance computing systems.

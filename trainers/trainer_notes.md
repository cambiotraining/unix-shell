...
- The introduction section of the slides covers [section 1](https://cambiotraining.github.io/unix-shell/materials/01-unix_overview.html) of the materials, i.e. a general introduction to Unix, terminal layout, basic command structure, manual page.
- You don't need to "live code" any of this section, but if participants want to open their terminal and try some of the commands shown on the slides, that's fine.
- Note that the terminal layout slides already mention `pwd`, `ls` and `cd` - but make sure to keep things light at this stage, don't go into too much detail about file paths, which are covered later. Make sure to highlight this to the participants, so they know more details will follow.
...
- There's only a couple of slides for this section to set the scene.
...
- There's only a couple of slides for this section to set the scene.
- This section naturally flows from the previous section and is also live-coded (often the same trainer does both).
...
- There's a few slides to start this section for a few key concepts.
- You can then live code the use of nano, and the script naturally follows from the previous section.
...
### Variables (no longer covered in the half-day workshop)

- This section follows naturally from the previous section and can be live coded.
...
### Loops (no longer covered in the half-day workshop)

[Loops](https://cambiotraining.github.io/unix-shell/materials/08-loops.html)
